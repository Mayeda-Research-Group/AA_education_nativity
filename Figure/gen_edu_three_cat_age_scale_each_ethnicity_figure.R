...
data_edu_nativity_cox[Variable== "College" &
                        Nativity=="Foreign-born" & 
                          Subpopulation=="Chinese",]$`95%.CI` # " (0.58, 0.9)"
data_edu_nativity_cox[Variable== "College" &
                        Nativity=="Foreign-born" & 
...
data_edu_nativity_cox[Variable== "College" &
                        Nativity=="US-born" & 
                        Subpopulation=="Japanese",]$`95%.CI` # " (0.77, 1.2)"
data_edu_nativity_cox[Variable== "College" &
                        Nativity=="US-born" & 
...
data_edu_nativity_cox[Variable== "Less than high school" &
                        Nativity=="Foreign-born" & 
                        Subpopulation=="Filipino",]$`95%.CI` # " (0.7, 1.2)"
data_edu_nativity_cox[Variable== "Less than high school" &
                        Nativity=="Foreign-born" & 
...
fig_nativity_cox_cat_asian <- ggplot(
  ...
  facet_nested(~factor(Subpopulation)+Nativity, scales = "free_x", space = "free_x") +
  ...
  scale_x_discrete(breaks = c("College", "High school (ref.)", "Less than high school"),
                   labels=c("College degree or higher", "High school degree, GED, \ntechnical/trade school, or some college", "Less than high school degree")) + 
  scale_y_log10(breaks=seq(0.5,1.5,by=0.1)) + # 12/07/2022
  xlab('') +
...
#save the result
ggsave(paste0(path_to_figures, "Education_nativity_three_cat_cox_2022_12_07.eps"), plot = fig_nativity_cox_cat_asian, width = 11.6, height = 8.2, units = "in")
ggsave(paste0(path_to_figures, "Education_nativity_three_cat_cox_2022_12_07.pdf"), plot = fig_nativity_cox_cat_asian, width = 11.6, height = 8.2, units = "in")
ggsave(paste0(path_to_figures, "Education_nativity_three_cat_aalen_2022_12_15.eps"), plot = fig_nativity_aalen_asian, width = 11.6, height = 8.2, units = "in")
ggsave(paste0(path_to_figures, "Education_nativity_three_cat_aalen_2022_12_15.pdf"), plot = fig_nativity_aalen_asian, width = 11.6, height = 8.2, units = "in")
