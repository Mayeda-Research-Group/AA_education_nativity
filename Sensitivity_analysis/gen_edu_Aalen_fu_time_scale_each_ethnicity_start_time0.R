...
# Adjustment: nativity, sex, survey_age
# Time scale: follow-up time
...
for(i in 1:max(data_post_imp$imp)){
  ...
  aalen_model_list_asian[[i]] <-
    aalen(Surv(main_dem_v1_fu_time, dem_end_flag) ~ 
            const(factor(college_edu))*const(factor(usaborn_rev)) + const(factor(female)) + const(age60),
          data=subset_asian, robust = TRUE, start.time = 0)
  
  aalen_model_list_chinese[[i]] <-
    aalen(Surv(main_dem_v1_fu_time, dem_end_flag) ~
            const(factor(college_edu))*const(factor(usaborn_rev)) + const(factor(female)) + const(age60),
          data=subset_chinese, robust = TRUE, start.time = 0)
  ...
  aalen_model_list_filipino[[i]] <-
    aalen(Surv(main_dem_v1_fu_time, dem_end_flag) ~
            const(factor(college_edu))*const(factor(usaborn_rev)) + const(factor(female)) + const(age60),
          data=subset_filipino, robust = TRUE, start.time = 0)
...
#save the result
aalen_result_list <- list(aalen_result_main_fu_time_scale = aalen_result,
                        aalen_result_raw_fu_time_scale = aalen_result_raw)

write.xlsx(aalen_result_list,
           file = paste0(path_to_output,"gen_edu_binary_aalen_fu_time_scale_2022_04_18_start_time0_v2.xlsx"),
           overwrite = TRUE)
