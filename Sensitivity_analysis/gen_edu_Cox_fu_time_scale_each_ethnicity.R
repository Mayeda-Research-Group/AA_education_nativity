...
for(i in 1:max(data_post_imp$imp)){
  ...
  cox_model_list_asian[[i]] <-
    coxph(Surv(main_dem_v1_fu_time, dem_end_flag) ~ 
            factor(college_edu) + factor(usaborn_rev) + factor(college_edu)*factor(usaborn_rev) + factor(female) + age60,
...
#save the result
cox_result_list <- list(cox_result_main_fu_time_scale = cox_result,
                        cox_result_raw_fu_time_scale = cox_result_raw)

write.xlsx(cox_result_list,
           file = paste0(path_to_output,"gen_edu_binary_cox_fu_time_scale_2022_04_18_v2.xlsx"),
           overwrite = TRUE)