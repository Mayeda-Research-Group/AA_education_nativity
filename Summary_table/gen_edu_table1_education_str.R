...
# Table1 for stacked imputed data, stratified by education using `college_edu` (04/20/2022)
...
#stratify by `college_edu` using `data_post_im` (04/20/2022)
...
# table(data_pre_im$college_edu, exclude = NULL) sanity check (RI 04/25/2022)
...
for (i in 1:length(contvars)){
  ...
  if (sum(is.na(eval(parse_expr(paste0("data_pre_im$",contvars[i]))))) >0){
    # table((is.na(data_pre_im$ehr_ht_median)),data_pre_im$college_edu) (RI 04/25/2022)
...
write.xlsx(t1_res_education_str,
           file = paste0(path_to_output, "gen_edu_Table1_education_str_raw_2022_04_25_v3.xlsx"),
           overwrite = TRUE)
