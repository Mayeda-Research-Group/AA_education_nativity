...
# Table1 for stacked imputed data, stratified by nativity (04/20/2022)
...
#stratify by `usaborn_rev` and `EDUCATION` using `data_post_im` (04/20/2022)
#collapse size of HH to alone/two/3+ (07/18/2022)
#collapse self-rated health to excellent/very good, good, and fair/poor (07/18/2022)
...
#collapsing sizeofhh and generalhealth (07/18/2022)
...
keepvars <- c("subjid", "survey_age", "female", "asian", "education_rev","education_cat", 
              ...
              "sizeofhh_coll", "generalhealth_coll", # added on 07/18/2022
...
data_pre_im$usaborn_rev <- as.character(data_pre_im$usaborn_rev)
data_pre_im[is.na(usaborn_rev), usaborn_rev:="missing"]
table(data_pre_im$usaborn_rev)
...
# table(data_pre_im$usaborn_rev, exclude = NULL) sanity check (RI 04/25/2022)

for (i in 1:length(catvars)){
  tab.to.add<-table(eval(rlang::parse_expr(paste0("data_pre_im$",catvars[i]))),
                    data_pre_im$usaborn_rev, exclude=NULL)
...
T1results_cat
colnames(T1results_cat)<-c("Variablename", "Asian_foreign_born", "Asian_US_born", "Asian_nativity_NA") 
...
T1results_prop[1,]<- c("Race/Ethnicity total",table(
  data_pre_im$usaborn_rev, exclude = NULL)/nrow(data_pre_im))

#Getting the denominators for the percentage calculations
Racethmargins <- as.numeric(table(data_pre_im$usaborn_rev, exclude = NULL))

for (i in 1:length(catvars)){
  tab.to.add<-t(t(table(eval(rlang::parse_expr(paste0("data_pre_im$",catvars[i])))
                        ,data_pre_im$usaborn_rev, exclude=NULL))/Racethmargins)
  labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
...
T1results_prop
colnames(T1results_prop)<-c("Variablename", "Asian_foreign_born_prop", "Asian_US_born_prop", "Asian_nativity_NA_prop") 
rownames(T1results_prop)<-NULL
T1results_prop<-as.data.frame(T1results_prop)
T1results_prop$Asian_foreign_born_prop <- paste0("(", 
                                    round(as.numeric(T1results_prop$Asian_foreign_born_prop)*100, 1),
                                    ")") #percentage value (03/30/2022)
T1results_prop$Asian_US_born_prop <- paste0("(", 
                                                 round(as.numeric(T1results_prop$Asian_US_born_prop)*100, 1),
                                                 ")") #percentage value (03/30/2022)
T1results_prop$Asian_nativity_NA_prop <- paste0("(", 
                                                 round(as.numeric(T1results_prop$Asian_nativity_NA_prop)*100, 1),
                                                 ")") #percentage value (03/30/2022)

#merge n and % results
T1results_cat <- left_join(T1results_cat, T1results_prop, by="Variablename")
T1results_cat <- T1results_cat[,c("Variablename",
                                "Asian_foreign_born", "Asian_foreign_born_prop",
                                "Asian_US_born", "Asian_US_born_prop",
                                "Asian_nativity_NA", "Asian_nativity_NA_prop")]
T1results_cat
...
colnames(T1results_cont)<-c("Variablename", "Asian_foreign_born", "Asian_US_born", "Asian_nativity_NA")

for (i in 1:length(contvars)){
  
  tab.to.add<-tapply(eval(parse_expr(paste0("data_pre_im$",contvars[i]))), 
                     data_pre_im$usaborn_rev, mean, na.rm=T)
  ...
  tab.to.add2<-tapply(eval(parse_expr(paste0("data_pre_im$",contvars[i]))), 
                      data_pre_im$usaborn_rev, sd, na.rm=T)
  ...
  if (sum(is.na(eval(parse_expr(paste0("data_pre_im$",contvars[i]))))) >0){
    # table((is.na(data_pre_im$ehr_ht_median)),data_pre_im$usaborn_rev) (RI 04/25/2022)
    tab.to.add3<-table(is.na(eval(parse_expr(paste0("data_pre_im$",contvars[i]))))
                       ,data_pre_im$usaborn_rev, exclude=NULL)["TRUE",]
  ...
  tab.to.add4<-tapply(eval(parse_expr(paste0("data_pre_im$",contvars[i]))), 
                      data_pre_im$usaborn_rev, median, na.rm=T)
  ...
  tab.to.add5<-tapply(eval(parse_expr(paste0("data_pre_im$",contvars[i]))), 
                      data_pre_im$usaborn_rev, quantile, probs=0.25, na.rm=T)
  ...
  tab.to.add6<-tapply(eval(parse_expr(paste0("data_pre_im$",contvars[i]))), 
                      data_pre_im$usaborn_rev, quantile, probs=0.75, na.rm=T)
...
T1results_cat_impute[1,]<- c("Race/ethnicity total",
                             table(data_post_im$usaborn_rev, exclude = NULL)/20)

for (i in 1:length(catvars)){
  tab.to.add<-table(eval(parse_expr(paste0("data_post_im$",catvars[i]))),
                    data_post_im$usaborn_rev, exclude=NULL)/20
  labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
...
colnames(T1results_cat_impute)<-c("Variablename", "Asian_foreign_born", "Asian_US_born") 
...
T1results_prop_impute[1,]<- c("Race/Ethnicity total",table(
  data_post_im$usaborn_rev)/nrow(data_post_im))

#Getting the denominators for the percentage calculations
Racethmargins<-as.numeric(table(data_post_im$usaborn_rev))

for (i in 1:length(catvars)){
  tab.to.add<-t(t(table(eval(parse_expr(paste0("data_post_im$",catvars[i])))
                        ,data_post_im$usaborn_rev, exclude=NULL))/Racethmargins)
  labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
...
colnames(T1results_prop_impute)<-c("Variablename", "Asian_foreign_born_prop", "Asian_US_born_prop") 
rownames(T1results_prop_impute)<-NULL
T1results_prop_impute<-as.data.frame(T1results_prop_impute)
T1results_prop_impute$`Asian_foreign_born_prop` <- paste0("(", 
                                           round(as.numeric(T1results_prop_impute$`Asian_foreign_born_prop`)*100, 1),
                                           ")") #percentage value (03/30/2022)
T1results_prop_impute$`Asian_US_born_prop` <- paste0("(", 
                                           round(as.numeric(T1results_prop_impute$`Asian_US_born_prop`)*100, 1),
                                           ")") #percentage value (03/30/2022)
...
table_nativity_cat_str_nonimp <- function(input){
  ...
  tmp_cat_substr[1,]<- c("Race/ethnicity total",
                               table(dt_input$usaborn_rev, exclude = NULL))
  
  for (i in 1:length(catvars)){
    tab.to.add<-table(eval(parse_expr(paste0("dt_input$",catvars[i]))),
                      dt_input$usaborn_rev, exclude=NULL)
    labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
  ...
  colnames(tmp_cat_substr)<-c("Variablename", paste0(input, "_foreign_born"),
                                    paste0(input, "_US_born"), paste0(input, "_NA")) 
  ...
  tmp_prop_substr[1,]<- c("Race/Ethnicity total",table(
    dt_input$usaborn_rev)/nrow(dt_input))
  
  #Getting the denominators for the percentage calculations
  Racethmargins<-as.numeric(table(dt_input$usaborn_rev))
  
  for (i in 1:length(catvars)){
    tab.to.add<-t(t(table(eval(parse_expr(paste0("dt_input$",catvars[i])))
                          ,dt_input$usaborn_rev, exclude=NULL))/Racethmargins)
    labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
  ...
  colnames(tmp_prop_substr)<-c("Variablename", paste0(input, "_foreign_born_prop"),
                                     paste0(input, "_US_born_prop"), paste0(input, "_NA_prop")) 
  ...
  tmp_cat_substr<-tmp_cat_substr[,c("Variablename", 
                                                paste0(input, "_foreign_born"), paste0(input, "_foreign_born_prop"),
                                                paste0(input, "_US_born"), paste0(input, "_US_born_prop"), 
                                                paste0(input, "_NA"), paste0(input, "_NA_prop"))]
  
  return(tmp_cat_substr)
  } 

T1results_cat_chinese <- table_nativity_cat_str_nonimp("Chinese") #86*7 (survey_language Spanish: 4)
T1results_cat_japanese <- table_nativity_cat_str_nonimp("Japanese") #85*7 (survey_language Spanish: 0)
T1results_cat_filipino <- table_nativity_cat_str_nonimp("Filipino") #85*7 (survey_language Chinese: 0)
...
T1results_cat_substr$Chinese_foreign_born_prop <- paste0("(",
                                                                round(as.numeric(T1results_cat_substr$Chinese_foreign_born_prop)*100, 1),
                                                                ")") #percentage value (03/30/2022)
T1results_cat_substr$Chinese_US_born_prop <- paste0("(",
                                                           round(as.numeric(T1results_cat_substr$Chinese_US_born_prop)*100, 1),
                                                           ")") #percentage value (03/30/2022)
T1results_cat_substr$Chinese_NA_prop <- paste0("(",
                                                    round(as.numeric(T1results_cat_substr$Chinese_NA_prop)*100, 1),
                                                    ")") #percentage value (03/30/2022)
T1results_cat_substr$Japanese_foreign_born_prop <- paste0("(",
                                                                 round(as.numeric(T1results_cat_substr$Japanese_foreign_born_prop)*100, 1),
                                                                 ")") #percentage value (03/30/2022)
T1results_cat_substr$Japanese_US_born_prop <- paste0("(",
                                                            round(as.numeric(T1results_cat_substr$Japanese_US_born_prop)*100, 1),
                                                            ")") #percentage value (03/30/2022)
T1results_cat_substr$Japanese_NA_prop <- paste0("(",
                                                     round(as.numeric(T1results_cat_substr$Japanese_NA_prop)*100, 1),
                                                     ")") #percentage value (03/30/2022)
T1results_cat_substr$Filipino_foreign_born_prop <- paste0("(",
                                                                 round(as.numeric(T1results_cat_substr$Filipino_foreign_born_prop)*100, 1),
                                                                 ")") #percentage value (03/30/2022)
T1results_cat_substr$Filipino_US_born_prop <- paste0("(",
                                                            round(as.numeric(T1results_cat_substr$Filipino_US_born_prop)*100, 1),
                                                            ")") #percentage value (03/30/2022)
...
table_nativity_cont_str_nonimp <- function(input){
  # dt_input <- data_pre_im[ethnicity_rev==2,]
  if(input == "Chinese"){ #ethnicity_rev == 2
    dt_input <- data_pre_im[ethnicity_rev==2,]
  }else if(input == "Japanese"){ #ethnicity_rev == 3
  ...
  colnames(tmp_cont_substr)<-c("Variablename", paste0(input, "_foreign_born"),
                                     paste0(input, "_US_born"), paste0(input, "_NA")) 
  
  for (i in 1:length(contvars)){
    
    tab.to.add<-tapply(eval(parse_expr(paste0("dt_input$",contvars[i]))), 
                       dt_input$usaborn_rev, mean, na.rm=T)
    ...
    tab.to.add2<-tapply(eval(parse_expr(paste0("dt_input$",contvars[i]))), 
                        dt_input$usaborn_rev, sd, na.rm=T)
    ...
    if (sum(is.na(eval(parse_expr(paste0("dt_input$",contvars[i]))))) >0){
      tab.to.add3<-table(is.na(eval(parse_expr(paste0("dt_input$",contvars[i]))))
                         ,dt_input$usaborn_rev, exclude=NULL)["TRUE",]
    ...
    tab.to.add4<-tapply(eval(parse_expr(paste0("dt_input$",contvars[i]))), 
                        dt_input$usaborn_rev, median, na.rm=T)
    ...
    tab.to.add5<-tapply(eval(parse_expr(paste0("dt_input$",contvars[i]))), 
                        dt_input$usaborn_rev, quantile, probs=0.25, na.rm=T)
    ...
    tab.to.add6<-tapply(eval(parse_expr(paste0("dt_input$",contvars[i]))), 
                        dt_input$usaborn_rev, quantile, probs=0.75, na.rm=T)
...
T1results_cont_chinese <- table_nativity_cont_str_nonimp("Chinese") #86*7 (survey_language Spanish: 4)
T1results_cont_japanese <- table_nativity_cont_str_nonimp("Japanese") #85*7 (survey_language Spanish: 0)
T1results_cont_filipino <- table_nativity_cont_str_nonimp("Filipino") #85*7 (survey_language Chinese: 0)
...
#Getting values for the imputed dataset; there should be no missingness!
#----Each ethnicity----
table_nativity_str <- function(input){
  if(input == "Chinese"){ #ethnicity_rev == 2
  ...
  T1_nativity_str<-matrix(nrow=1, ncol=3) 
  T1_nativity_str[1,]<- c("Race/ethnicity total",
                               table(dt_input$usaborn_rev, exclude = NULL)/20)
  
  for (i in 1:length(catvars)){
    tab.to.add<-table(eval(parse_expr(paste0("dt_input$",catvars[i]))),
                      dt_input$usaborn_rev, exclude=NULL)/20
    labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
    T1_nativity_str<-rbind(T1_nativity_str, c(paste(catvars[i]), rep(NA,2))) 
    T1_nativity_str<-rbind(T1_nativity_str,cbind(labs, tab.to.add))
  }
  #Notes: parse_expr() spits out the string as an expression. Must use eval() fxn
  #in order to use the expression (if eval() removed, will cause an error)
  
  colnames(T1_nativity_str)<-c("Variablename", paste0(input, "_foreign_born"), paste0(input, "_US_born")) 
  rownames(T1_nativity_str)<-NULL
  T1_nativity_str<-as.data.frame(T1_nativity_str)
  
  #Get %'s by race/ethnicity
  T1_prop_nativity_str<-matrix(nrow=1, ncol=3) 
  T1_prop_nativity_str[1,]<- c("Race/Ethnicity total",table(
    dt_input$usaborn_rev)/nrow(dt_input))
  
  #Getting the denominators for the percentage calculations
  Margins<-as.numeric(table(dt_input$usaborn_rev))
  
  for (i in 1:length(catvars)){
    tab.to.add<-t(t(table(eval(parse_expr(paste0("dt_input$",catvars[i])))
                          ,dt_input$usaborn_rev, exclude=NULL))/Margins)
    labs<-paste(catvars[i],as.character(rownames(tab.to.add)))
    T1_prop_nativity_str<-rbind(T1_prop_nativity_str, c(paste(catvars[i]),rep(NA,2))) 
    T1_prop_nativity_str<-rbind(T1_prop_nativity_str,cbind(labs, tab.to.add))
  }
  
  colnames(T1_prop_nativity_str)<-c("Variablename", paste0(input, "_foreign_born_prop"), paste0(input, "_US_born_prop")) 
  rownames(T1_prop_nativity_str)<-NULL
  T1_prop_nativity_str<-as.data.frame(T1_prop_nativity_str)
  
  #merge n and % results
  T1_nativity_str<-left_join(T1_nativity_str, 
                                  T1_prop_nativity_str, by="Variablename")
  T1_nativity_str<-T1_nativity_str[,c("Variablename",
                                                paste0(input, "_foreign_born"), paste0(input, "_foreign_born_prop"),
                                                paste0(input, "_US_born"), paste0(input, "_US_born_prop"))]
  
  return(T1_nativity_str)
}

T1results_cat_impute_chinese <- table_nativity_str("Chinese")
T1results_cat_impute_japanese <- table_nativity_str("Japanese")
T1results_cat_impute_filipino <- table_nativity_str("Filipino")
...
T1results_cat_impute_substr$Chinese_foreign_born_prop <- paste0("(",
                                                                round(as.numeric(T1results_cat_impute_substr$Chinese_foreign_born_prop)*100, 1),
                                                                ")") #percentage value (03/30/2022)
T1results_cat_impute_substr$Chinese_US_born_prop <- paste0("(",
                                                      round(as.numeric(T1results_cat_impute_substr$Chinese_US_born_prop)*100, 1),
                                                      ")") #percentage value (03/30/2022)
T1results_cat_impute_substr$Japanese_foreign_born_prop <- paste0("(",
                                                                round(as.numeric(T1results_cat_impute_substr$Japanese_foreign_born_prop)*100, 1),
                                                                ")") #percentage value (03/30/2022)
T1results_cat_impute_substr$Japanese_US_born_prop <- paste0("(",
                                                      round(as.numeric(T1results_cat_impute_substr$Japanese_US_born_prop)*100, 1),
                                                      ")") #percentage value (03/30/2022)
T1results_cat_impute_substr$Filipino_foreign_born_prop <- paste0("(",
                                                                round(as.numeric(T1results_cat_impute_substr$Filipino_foreign_born_prop)*100, 1),
                                                                ")") #percentage value (03/30/2022)
T1results_cat_impute_substr$Filipino_US_born_prop <- paste0("(",
                                                      round(as.numeric(T1results_cat_impute_substr$Filipino_US_born_prop)*100, 1),
                                                      ")") #percentage value (03/30/2022)
T1results_cat_impute_substr
...
for(var in c("main_dem_v1_fu_time", "survey_age", "income_pp", "ehr_ht_median")){
  ...
  # mean(AA_data, na.rm = T) # Sporadic NA's due to MI (04/20/2022)
  ...
  cont_results_US_born[which(cont_results_US_born$var == var & cont_results_US_born$stat == "mean"), 
               "Asian"] <- mean(AA_data, na.rm = T)
  cont_results_US_born[which(cont_results_US_born$var == var & cont_results_US_born$stat == "SD"), 
               "Asian"] <- sd(AA_data, na.rm = T)
  cont_results_US_born[which(cont_results_US_born$var == var & cont_results_US_born$stat == "median"), 
               "Asian"] <- median(AA_data, na.rm = T)
  cont_results_US_born[which(cont_results_US_born$var == var & cont_results_US_born$stat == "q1"), 
               "Asian"] <- quantile(AA_data, probs = 0.25, na.rm = T)
  cont_results_US_born[which(cont_results_US_born$var == var & cont_results_US_born$stat == "q3"), 
  ...
  for(group in names(table(subset$ethnicity_rev))){
    ...
    # subgroup_imp_mean = rowMeans(subgroup_data) # Sporadic NA's due to MI (04/20/2022)
    cont_results_US_born[which(cont_results_US_born$var == var & cont_results_US_born$stat == "mean"), 
...
colnames(cont_results_US_born) <- c("var","stat","Asian_US_born","Chinese_US_born", 
                          "Japanese_US_born","Filipino_US_born")
cont_results_US_born$concat <- paste(cont_results_US_born$var, sep = "_", cont_results_US_born$stat)
cont_results_US_born

# Foreign-born
cont_results_foreign_born <- 
  tibble("var" = 
           rep(c("main_dem_v1_fu_time", "survey_age", "income_pp", "ehr_ht_median"), 
               each = 5), 
         "stat" = rep(c("mean", "SD", "median", "q1", "q3"), 4), 
...
cont_results_foreign_born <- as.data.frame(cont_results_foreign_born)

for(var in c("main_dem_v1_fu_time", "survey_age", "income_pp", "ehr_ht_median")){
  # subset_test <- data_post_im[usaborn_rev==0,] %>% 
  #   dplyr::select("subjid", "imp", "asian", "ethnicity_rev", "main_dem_v1_fu_time")
  
  subset <- data_post_im[usaborn_rev==0,] %>% 
    dplyr::select("subjid", "imp", "asian", "ethnicity_rev", all_of(var))
  ...
  # statistics over all matrix elements excluding NAs (04/22/2022)
  cont_results_foreign_born[which(cont_results_foreign_born$var == var & cont_results_foreign_born$stat == "mean"), 
                       "Asian"] <- mean(AA_data, na.rm = T)
  cont_results_foreign_born[which(cont_results_foreign_born$var == var & cont_results_foreign_born$stat == "SD"), 
                       "Asian"] <- sd(AA_data, na.rm = T)
  cont_results_foreign_born[which(cont_results_foreign_born$var == var & cont_results_foreign_born$stat == "median"), 
                       "Asian"] <- median(AA_data, na.rm = T)
  cont_results_foreign_born[which(cont_results_foreign_born$var == var & cont_results_foreign_born$stat == "q1"), 
                       "Asian"] <- quantile(AA_data, probs = 0.25, na.rm = T)
  cont_results_foreign_born[which(cont_results_foreign_born$var == var & cont_results_foreign_born$stat == "q3"), 
                       "Asian"] <- quantile(AA_data, probs = 0.75, na.rm = T)
...
colnames(cont_results_foreign_born) <- c("var","stat","Asian_foreign_born","Chinese_foreign_born", 
                                    "Japanese_foreign_born","Filipino_foreign_born")
# cont_results_foreign_born <- cont_results_foreign_born %>% select(-c("var", "stat"))

cont_results <- merge(cont_results_US_born, cont_results_foreign_born, by = c("var", "stat")) 
cont_results <- cont_results %>% select(c("var", "stat", "concat",
                                          "Asian_foreign_born", "Asian_US_born",
                                          "Chinese_foreign_born", "Chinese_US_born",
                                          "Japanese_foreign_born", "Japanese_US_born",
                                          "Filipino_foreign_born", "Filipino_US_born"))
cont_results

#----Export results to a raw Excel----
t1_res_nativity_str <- 
  list(cat_nativity=T1results_cat, 
             cont_nativity=T1results_cont, 
             cat_nativity_imp=T1results_cat_impute,
             cat_substr_nativity = T1results_cat_substr,
             cont_substr_nativity = T1results_cont_substr,
             cat_nativity_imp_substr = T1results_cat_impute_substr,
             cont_nativity_imp_all = cont_results)

write.xlsx(t1_res_nativity_str,
           file = paste0(path_to_output, "gen_edu_Table1_nativity_str_raw_2022_04_25_v3.xlsx"),
           overwrite = TRUE)
