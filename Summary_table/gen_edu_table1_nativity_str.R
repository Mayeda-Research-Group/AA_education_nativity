...
# Table1 for stacked imputed data, stratified by nativity
...
#stratify by `usaborn_rev` and `EDUCATION` using `data_post_im` 
...
# table(data_pre_im$usaborn_rev, exclude = NULL) sanity check
...
for (i in 1:length(contvars)){
  ...
  if (sum(is.na(eval(parse_expr(paste0("data_pre_im$",contvars[i]))))) >0){
    # table((is.na(data_pre_im$ehr_ht_median)),data_pre_im$usaborn_rev)
...
for(var in c("main_dem_v1_fu_time", "survey_age", "income_pp", "ehr_ht_median")){
  ...
  # statistics over all matrix elements excluding NAs
...
write.xlsx(t1_res_nativity_str,
           file = paste0(path_to_output, "gen_edu_Table1_nativity_str_raw_2022_12_07.xlsx"),
           overwrite = TRUE)
